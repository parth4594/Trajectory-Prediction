...
def means_and_stds(train_dataset, feature_count):
    ...
    for i in np.unique(train_dataset[:]['dataset']):
        ...
        input_src_mean = torch.cat((input_src, input_trg), 1).mean(
            (0, 1))  # find mean across zero axis first then find the mean across axis 1
...
def parser_function():
    ...
    parser.add_argument('--csv_path', type=str,
                        default='../Traj_Transformer/Datasets/Original Datasets/dataset_03_red/')
    parser.add_argument('--dataset_folder', type=str, default='../Traj_Transformer/Datasets/Trasformed Datasets/')
    parser.add_argument('--dataset_name', type=str, default='RounD_new_red')
...
def data_generator(args):
    ...
    trans_dataset_gen(0.3,
                      '/home/parth/traffic_data_fussion/Traj_Transformer/Datasets/Original Datasets/dataset_03_red/',
                      '/home/parth/traffic_data_fussion/Traj_Transformer/Datasets/Transformed Datasets/',
                      "RounD_new_img_8", "03",
                      '/home/parth/traffic_data_fussion/Local Map Generation/Local_Maps_traj/')
    ...
    if available:
        ...
        train_dataset, input_mean, input_std, target_mean, target_std = baselineUtils_transformed.create_dataset(
            args.dataset_folder, args.dataset_name, 0, args.obs, args.preds, train=True, eval=False, verbose=False)
        val_dataset, input_mean_v, input_std_v, target_mean_v, target_std_v = baselineUtils_transformed.create_dataset(
            args.dataset_folder, args.dataset_name, 0, args.obs,
    ...
    return train_dataset, val_dataset, input_mean, input_std, target_mean, target_std
    ###################################################################################################################
...
def objective(trial):
    ...
    train_dataset, val_dataset, input_mean, input_std, target_mean, target_std = data_generator(args)
    log = log_function(args)
    ...
    scipy.io.savemat(f'models/Individual/{args.name}/norm.mat',
                     {'mean': input_mean.cpu().numpy(), 'std': input_std.cpu().numpy()})
    ...
    model = IndividualTF(feature_count, args.dec_in, args.dec_out, N=args.layers, in_channels=3,
                         pretrained=args.pretrained,  ##change
                         d_model=args.emb_size, d_ff=args.dff, h=args.heads, dropout=args.dropout, mean=[0, 0],
                         std=[0, 0]).to(device)  # to(device) means to transfer a tensor to a GPU
    if args.resume_train:
        model.load_state_dict(torch.load(
            f'models/Individual/{args.name}/{args.model_pth}'))  # A state_dict is simply a Python dictionary object that maps each layer to its parameter tensor.
    ...
    next_row = {"date": save_time, "current_epoch": 0, "max_epoch": args.max_epoch, "training_time": 0.0,
                "total_loss": 0.0, "agv_loss": 0.0, "mad": 0.0, "fad": 0.0,
    ...
    while val_rel_err > val_rel_err_thresh and epoch < args.max_epoch:
        ...
        for id_b, batch in enumerate(tr_dl):
            ...
            # print('img \n', inp_img)
            # input('stop')
            target = (batch['trg'][:, :-1, 0:2].to(device) - target_mean.to(device)) / target_std.to(
                device)  # change  #our target is the coordinates why -1?
            # print('target0 \n',target[0])
            # input("stop2")
            ...
            y_pred = pred[:, :, 0:2].contiguous().view(-1,
                                                       2)  # predicts 3 values. X and Y coordinates and an end token. We only select coordinates
            # print('y_pred \n',y_pred)
            # print('y_pred_size \n',y_pred.shape)
            y_real = ((batch['trg'][:, :, 0:2].to(device) - target_mean.to(device)) / target_std.to(
                device)).contiguous().view(-1, 2).to(device)  # change
            # print('y_real \n',y_real)
            # print('y_real_size \n',y_real.shape)
            # input("stop4")
        ...
        with torch.no_grad():
            ...
            for id_b, batch in enumerate(val_dl):
                ...
                # input("stop5")
                inp = (batch['src'][:, :, 0:feature_count].to(device) - input_mean.to(device)) / input_std.to(
                    device)  # change
                # print('inp_val \n', inp[0].shape)
                inp_img = batch['maps'].to(device)
                src_att = torch.ones((inp.shape[0], 1, inp.shape[1])).to(device)
                ...
                preds_tr_b = (dec_inp[:, 1:, 0:2] * target_std.to(device) + target_mean.to(
                    device)).cpu()  # .numpy() .cumsum(1) + batch['src'][:, -1:, 0:2].cpu().numpy() #'1'dec_inp means we are leaving out the token
                # print('predicted_val \n', preds_tr_b[0])
            ...
            update = {"current_epoch": epoch, "training_time": train_time, "total_loss": epoch_loss,
                      "agv_loss": avg_loss, "mad": mad, "fad": fad}
            ...
            if epoch % args.save_step == 0:
                print(
                    f"Epoch: {epoch:03d} Training time: {train_time}  Loss: {epoch_loss:03.4f}  Avg. Loss: {avg_loss:03.4f} MAD: {mad:03.4f} FAD: {fad:03.4f}")
            ...
            if epoch % epoch_check_freq == 0:
                if epoch > epoch_check_freq:
                    ...
                    print(
                        f'val_err_min: {val_err_min:03.4f} curr_val_err_min: {curr_val_err_min:03.4f} val_rel_err: {val_rel_err:03.4f}')
                ...
                if val_rel_err < val_rel_err_thresh:
                    print(
                        f'Reached less than {val_rel_err_thresh} val error change over last {epoch_check_freq} epochs. Stopping training')
